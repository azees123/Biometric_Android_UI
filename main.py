...
class FingerprintApp(App):
    def build(self):
        ...
        # Register button
        register_btn = MDRaisedButton(text="Register", size_hint=(1, 0.2))
        register_btn.bind(on_press=self.open_register_popup)
        layout.add_widget(register_btn)

        # Verify button
        verify_btn = MDRaisedButton(text="Verify", size_hint=(1, 0.2))
        verify_btn.bind(on_press=self.open_verify_popup)
        layout.add_widget(verify_btn)

        # Label for displaying verification result
        self.result_label = MDLabel(text="Verification Status will be shown here.", size_hint=(1, 0.1))
    ...
    def open_register_popup(self, instance):
        ...
        self.name_input = MDTextField(hint_text="Enter Name", size_hint=(1, None), height=40)
        self.emp_id_input = MDTextField(hint_text="Enter Employee ID", size_hint=(1, None), height=40)
        self.phone_input = MDTextField(hint_text="Enter Phone Number", size_hint=(1, None), height=40)

        self.select_fp_btn = MDRaisedButton(text="Select Fingerprint Image", size_hint=(1, None), height=40)
        self.select_fp_btn.bind(on_press=self.select_fingerprint)

        submit_btn = MDRaisedButton(text="Submit", size_hint=(1, None), height=50)
        ...
        self.popup = MDDialog(title="Register User", content=self.reg_layout, size_hint=(0.9, 0.8), auto_dismiss=False)
        self.popup.open()
